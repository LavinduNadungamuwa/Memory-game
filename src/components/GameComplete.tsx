...
const GameComplete: React.FC<GameCompleteProps> = ({ moves, time, onRestart }) => {
  ...
  const getPerformanceRating = () => {
    if (moves <= 20 && time <= 60) return { stars: 3, message: "Incredible!" };
    if (moves <= 30 && time <= 90) return { stars: 2, message: "Great job!" };
